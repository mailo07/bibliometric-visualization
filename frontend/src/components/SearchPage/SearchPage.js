...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    try {
      const results = await search(searchTerm, 'all', page, resultsPerPage, true);
    ...
    setData(resultsWithIds);
      ...
      setMetrics({
        ...
        worksCited: results.metrics?.worksCited || results.reduce((sum, item) => sum + (item.citation_count || 0), 0),
        frequentlyCited: results.metrics?.frequentlyCited || results.filter(item => (item.citation_count || 0) > 10).length
      });
    } catch (err) {
...
const handleResetFilters = () => {
    setCurrentFilters({ authorFilter: '', subjectFilter: '', dateRange: { start: '', end: '' }, journalFilter: '',
      titleFilter: '', publisherFilter: '', identifierTypes: { doi: false, pmid: false, pmcid: false,
        arxiv: false, isbn: false }
    });
    setCurrentPage(1);
  };
const handleTabSwitch = (tab) => {
    if (activeTab !== tab) { setTabTransition(true);
      setTimeout(() => { setActiveTab(tab);
        setTimeout(() => { setTabTransition(false);
        }, 400);
      }, 400);
    }
  };
const renderIdentifiers = (identifiers) => { if (!identifiers) return 'N/A';
    ...
    return parts.map((part, index) => {
      ...
      return ( <React.Fragment key={index}> {index > 0 && ', '} {part}
        </React.Fragment>
      );
    });
  };
const generateBibliometricCharts = useCallback((searchResults) => { return bibliometricCharts;}, [bibliometricCharts]);
useEffect(() => {
    if (data.length > 0) { const finalCharts = generateBibliometricCharts(data);
...
<div className="flex flex-grow relative">
        ...
        <main className={`main-column p-6`}>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 bg-white/90 shadow-md p-4 rounded divide-x-2 divide-gray-200">
            <div className="text-center"> <h3 className="font-semibold text-purple-800">Scholarly Works</h3>
              <p className="text-2xl font-bold">{metrics.scholarlyWorks}</p> </div>
            <div className="text-center"> <h3 className="font-semibold text-purple-800">Works Cited</h3>
              <p className="text-2xl font-bold">{metrics.worksCited}</p> </div>
            <div className="text-center"> <h3 className="font-semibold text-purple-800">Frequently Cited Works</h3>
              <p className="text-2xl font-bold">{metrics.frequentlyCited}</p></div></div>
          <div className="mb-6 bg-white/90 shadow-md p-4 rounded">
            ...
            <p className="text-gray-700"> Total Results: <span className="font-medium">{filteredData.length}</span></p></div>
              ...
              <div className={`results-container bg-white/90 shadow-md rounded p-4 overflow-x-auto transition-opacity duration-800 ease-in-out ${ resultsTransition ? 'opacity-0' : 'opacity-100' }`}>
                {filteredData.length > 0 ? (
                  <> <table className="w-full table-auto border-collapse">
                    ...
                    <Pagination initialPage={currentPage} totalItems={filteredData.length} itemsPerPage={resultsPerPage} onPageChange={handlePageChange} transitionEnabled={true}/>
                  </> ) : ( query && ( <p className="text-gray-600 text-center"> No results found for your search query</p>))}
              </div> )}
            {!loading && !error && activeTab === 'citation' && (<SummaryTab selectedWork={selectedWork} resetSelectedWork={resetSelectedWork} />)}
            {!loading && !error && activeTab === 'analysis' && (<div className="bg-white/90 shadow-md rounded p-4"> <h3 className="font-semibold text-purple-800 mb-4">Research Network</h3> <NetworkGraph searchResults={data} /></div>
            )}</div></main>
        ...
        <footer className="bg-gradient-to-r from-indigo-900 to-purple-900 text-white text-center py-10"> <p className="text-gray-300 text-sm"> This website is just an educational project and is not meant for intended use. </p>
      </footer>
    </div>
  );
};
